...
export function NewsletterForm({ tone = 'auto' }: { tone?: 'auto' | 'inverse' }) {
  ...
  const text =
    tone === 'inverse' ? 'text-white' : 'text-black dark:text-white'
  const border =
  ...
  if (status === 'success') {
    return (
      <p className={`font-mono text-xs uppercase tracking-widest ${text} opacity-60`}>
        you&apos;re in.
      </p>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} className={`flex w-full items-center border-b ${border}`}>
      <input
        type="email"
        value={email}
